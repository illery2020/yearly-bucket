...
const USERS = ["たかひろ", "パートナー"];

export default function Home() {
  const [items, setItems] = useState([]);
  const [isLoaded, setIsLoaded] = useState(false);
  const [currentUser, setCurrentUser] = useState(USERS[0]);
  const [viewMode, setViewMode] = useState("all"); // "all", "たかひろ", "パートナー"
  ...
  return (
    <main className="min-h-screen pb-10">
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 mb-8 flex flex-col md:flex-row gap-6 items-center justify-between">
        ...
        {/* List Filtering - Whose items to show? */}
        <Tabs value={viewMode} onValueChange={setViewMode} className="w-auto">
          <TabsList className="bg-slate-800/50 border border-slate-700/50 p-1 rounded-full h-auto">
            ...
            {USERS.map(user => (
              <TabsTrigger key={user} value={user} className="rounded-full px-6 py-2">
                {user}
              </TabsTrigger>
            ))}
          </TabsList>
